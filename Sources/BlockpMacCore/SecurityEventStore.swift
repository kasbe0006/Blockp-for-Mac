...
public struct SecurityEventStore: Sendable {
    ...
    public func append(_ event: SecurityEvent) {
        let line = "\(Self.iso8601.string(from: event.timestamp))|\(event.severity.rawValue)|\(event.source)|\(sanitize(event.message))\n"
    ...
    private func parseLine(_ line: String) -> SecurityEvent? {
        ...
        guard let timestamp = Self.iso8601.date(from: parts[0]),
